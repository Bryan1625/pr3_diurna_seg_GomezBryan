...
public class TransaccionController {
    ...
    public void onRealizarTransaccion1Click(ActionEvent actionEvent) {
        ...
        if (cbTipoTransaccion.getValue().equals("Solicitud")) {
            transaccion = new Solicitud(Double.parseDouble(txtValorTransaccion.getText()), LocalDate.now().toString(), LocalTime.now().toString(), Estado.Exitosa);
            cuenta.setSaldo(cuenta.getSaldo() + transaccion.getValor());
        } else if (cbTipoTransaccion.getValue().equals("Deposito")) {
            transaccion = new Deposito(Double.parseDouble(txtValorTransaccion.getText()), LocalDate.now().toString(), LocalTime.now().toString(), Estado.Exitosa);
            cuenta.setSaldo(cuenta.getSaldo() + transaccion.getValor());
        }else if (cbTipoTransaccion.getValue().equals("Retiro")) {
            if (cuenta.getSaldo()>= Double.parseDouble(txtValorTransaccion.getText())) {
                transaccion = new Retiro(Double.parseDouble(txtValorTransaccion.getText()), LocalDate.now().toString(), LocalTime.now().toString(), Estado.Exitosa);
                cuenta.setSaldo(cuenta.getSaldo()-transaccion.getValor());
            }else{
                ...
                alert.showAndWait();
                transaccion = new Solicitud(Double.parseDouble(txtValorTransaccion.getText()), LocalDate.now().toString(), LocalTime.now().toString(), Estado.SinFondos);
            }
        }
    }
